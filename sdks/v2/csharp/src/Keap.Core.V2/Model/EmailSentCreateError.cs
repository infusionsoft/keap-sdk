/*
 * Keap API
 *
 * Keap Public API Documentation
 *
 * The version of the OpenAPI document: v2
 * Contact: api.keap@thryv.com
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = Keap.Core.V2.Client.OpenAPIDateConverter;

namespace Keap.Core.V2.Model
{
    /// <summary>
    /// Error that occurred when creating an email record
    /// </summary>
    [DataContract(Name = "EmailSentCreateError")]
    public partial class EmailSentCreateError : IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="EmailSentCreateError" /> class.
        /// </summary>
        /// <param name="errorMessage">Description of the error.</param>
        /// <param name="email">The email request that failed.</param>
        public EmailSentCreateError(string errorMessage = default, CreateEmailSentRequest email = default)
        {
            this.ErrorMessage = errorMessage;
            this.Email = email;
        }

        /// <summary>
        /// Description of the error
        /// </summary>
        /// <value>Description of the error</value>
        /*
        <example>Contact not found</example>
        */
        [DataMember(Name = "error_message", EmitDefaultValue = false)]
        public string ErrorMessage { get; set; }

        /// <summary>
        /// The email request that failed
        /// </summary>
        /// <value>The email request that failed</value>
        [DataMember(Name = "email", EmitDefaultValue = false)]
        public CreateEmailSentRequest Email { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class EmailSentCreateError {\n");
            sb.Append("  ErrorMessage: ").Append(ErrorMessage).Append("\n");
            sb.Append("  Email: ").Append(Email).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }    }

}
