...
export class PipelinesApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param pageSize 
     */
    public async getDealCount(id: string, filter?: string, pageToken?: string, orderBy?: string, pageSize?: number, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'id' is not null or undefined
        if (id === null || id === undefined) {
            throw new RequiredError("PipelinesApi", "getDealCount", "id");
        }
        ...
        // Path Params
        const localVarPath = '/v2/pipelines/{id}:dealsCount'
            .replace('{' + 'id' + '}', encodeURIComponent(String(id)));
...
export class PipelinesApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getDealCount
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getDealCountWithHttpInfo(response: ResponseContext): Promise<HttpInfo<number >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
