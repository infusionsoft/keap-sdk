...
export class ReportingApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param reportId The unique identifier of the report (Saved Search)
     */
    public async retrieveReportModel(reportId: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'reportId' is not null or undefined
        if (reportId === null || reportId === undefined) {
            throw new RequiredError("ReportingApi", "retrieveReportModel", "reportId");
        }


        // Path Params
        const localVarPath = '/rest/v2/reporting/reports/{report_id}/model'
            .replace('{' + 'report_id' + '}', encodeURIComponent(String(reportId)));
...
export class ReportingApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to retrieveReportModel
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async retrieveReportModelWithHttpInfo(response: ResponseContext): Promise<HttpInfo<ReportModel >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: ReportModel = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "ReportModel", ""
            ) as ReportModel;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
