...
export class ReferralApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async listReferrals(filter?: string, pageToken?: string, orderBy?: string, pageSize?: number, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;
...
export class ReferralApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to listReferrals
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async listReferralsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<ListReferralsResponse >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: ListReferralsResponse = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "ListReferralsResponse", ""
            ) as ListReferralsResponse;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
        }
        if (isCodeInRange("400", response.httpStatusCode)) {
            const body: Error = ObjectSerializer.deserialize(
