...
public class AddProductInterestRequest implements Serializable {
  ...
  @Schema(requiredMode = Schema.RequiredMode.REQUIRED, description = "The `product_id` (when the `type` is set to `PRODUCT`)  or the `subscription_id` (when the `type` is set to `SUBSCRIPTION_PLAN`). ")
