/**
 * Keap REST API
 * Keap REST Core Service
 *
 * The version of the OpenAPI document: 2.70.0.872719-hf-202511041049
 * Contact: api@keap.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import CurrencyValue from './CurrencyValue';

/**
 * The ProductInterest model module.
 * @module keap.core.v2/model/ProductInterest
 */
class ProductInterest {
    /**
     * Constructs a new <code>ProductInterest</code>.
     * @alias module:keap.core.v2/model/ProductInterest
     */
    constructor() { 
        
        ProductInterest.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>ProductInterest</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:keap.core.v2/model/ProductInterest} obj Optional instance to populate.
     * @return {module:keap.core.v2/model/ProductInterest} The populated <code>ProductInterest</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new ProductInterest();

            if (data.hasOwnProperty('discount_percent')) {
                obj['discount_percent'] = ApiClient.convertToType(data['discount_percent'], 'Number');
            }
            if (data.hasOwnProperty('id')) {
                obj['id'] = ApiClient.convertToType(data['id'], 'String');
            }
            if (data.hasOwnProperty('interest_id')) {
                obj['interest_id'] = ApiClient.convertToType(data['interest_id'], 'String');
            }
            if (data.hasOwnProperty('price')) {
                obj['price'] = CurrencyValue.constructFromObject(data['price']);
            }
            if (data.hasOwnProperty('quantity')) {
                obj['quantity'] = ApiClient.convertToType(data['quantity'], 'Number');
            }
            if (data.hasOwnProperty('type')) {
                obj['type'] = ApiClient.convertToType(data['type'], 'String');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>ProductInterest</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>ProductInterest</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['id'] && !(typeof data['id'] === 'string' || data['id'] instanceof String)) {
            throw new Error("Expected the field `id` to be a primitive type in the JSON string but got " + data['id']);
        }
        // ensure the json data is a string
        if (data['interest_id'] && !(typeof data['interest_id'] === 'string' || data['interest_id'] instanceof String)) {
            throw new Error("Expected the field `interest_id` to be a primitive type in the JSON string but got " + data['interest_id']);
        }
        // validate the optional field `price`
        if (data['price']) { // data not null
          CurrencyValue.validateJSON(data['price']);
        }
        // ensure the json data is a string
        if (data['type'] && !(typeof data['type'] === 'string' || data['type'] instanceof String)) {
            throw new Error("Expected the field `type` to be a primitive type in the JSON string but got " + data['type']);
        }

        return true;
    }


}



/**
 * The percent to discount the product. Percent represented in a whole number, for example 10 is 10%
 * @member {Number} discount_percent
 */
ProductInterest.prototype['discount_percent'] = undefined;

/**
 * The product interest ID
 * @member {String} id
 */
ProductInterest.prototype['id'] = undefined;

/**
 * The interest ID
 * @member {String} interest_id
 */
ProductInterest.prototype['interest_id'] = undefined;

/**
 * @member {module:keap.core.v2/model/CurrencyValue} price
 */
ProductInterest.prototype['price'] = undefined;

/**
 * The quantity of product.
 * @member {Number} quantity
 */
ProductInterest.prototype['quantity'] = undefined;

/**
 * The product interest type
 * @member {module:keap.core.v2/model/ProductInterest.TypeEnum} type
 */
ProductInterest.prototype['type'] = undefined;





/**
 * Allowed values for the <code>type</code> property.
 * @enum {String}
 * @readonly
 */
ProductInterest['TypeEnum'] = {

    /**
     * value: "PRODUCT"
     * @const
     */
    "PRODUCT": "PRODUCT",

    /**
     * value: "SUBSCRIPTION_PLAN"
     * @const
     */
    "SUBSCRIPTION_PLAN": "SUBSCRIPTION_PLAN",

    /**
     * value: "unknown_default_open_api"
     * @const
     */
    "unknown_default_open_api": "unknown_default_open_api"
};


export default ProductInterest;
