...
let applyRemoveTagRequest = new KeapCoreServiceV2Sdk.ApplyRemoveTagRequest(); // ApplyRemoveTagRequest | 
...
> TagContactCount countContactsForTag(tagId, opts)
...
let apiInstance = new KeapCoreServiceV2Sdk.TagsApi();
let tagId = "tagId_example"; // String | The ID of the tag whose contacts are counted
let opts = {
...
apiInstance.countContactsForTag(tagId, opts).then((data) => {
  console.log('API called successfully. Returned data: ' + data);
}, (error) => {
  console.error(error);
});
...
let opts = {
  ...
  'pageToken': "pageToken_example", // String | Page token
  'orderBy': "orderBy_example", // String | Attribute and direction to order items (best-effort for cross-collection reads). Field: - `applied_time`  Direction: - `asc` - `desc`  Field and direction are space-separated, e.g. `applied_time desc`.
  'pageSize': 0 // Number | Total number of items to return per page
};
...
> UpdateTagResponse updateTag(tagId, createUpdateTagRequest, opts)
...
> UpdateTagCategoryResponse updateTagCategory(tagCategoryId, createUpdateTagCategoryRequest, opts)
